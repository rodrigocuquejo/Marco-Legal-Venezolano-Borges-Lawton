...
# FUNCIÓN AUTO-DETECTABLE ADAPTATIVA PARA BOTONES
def obtener_leyes_por_categoria(categoria_nombre):
    if not os.path.exists(DB_NAME):
        st.error(f"❌ Archivo de base de datos '{DB_NAME}' no encontrado en el servidor.")
        return []
    try:
        conn = sqlite3.connect(DB_NAME)
        cursor = conn.cursor()
        
        # 1. Detectar el nombre real de la tabla
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
        tablas = [t[0] for t in cursor.fetchall() if t[0] != 'sqlite_sequence']
        if not tablas:
            st.error("❌ La base de datos está vacía (no contiene tablas).")
            conn.close()
            return []
        tabla_actual = 'leyes' if 'leyes' in tablas else tablas[0]
        
        # 2. Detectar dinámicamente las columnas
        cursor.execute(f"PRAGMA table_info({tabla_actual})")
        columnas = [col[1] for col in cursor.fetchall()]
        
        col_id = 'id' if 'id' in columnas else columnas[0]
        col_nombre = 'nombre' if 'nombre' in columnas else ('titulo' if 'titulo' in columnas else columnas[1])
        col_cat = 'categoria' if 'categoria' in columnas else ('area' if 'area' in columnas else None)
        col_subcat = 'subcategoria' if 'subcategoria' in columnas else ('sub_categoria' if 'sub_categoria' in columnas else None)
        col_enlace = 'enlace_pdf' if 'enlace_pdf' in columnas else ('enlace' if 'enlace' in columnas else ('link' if 'link' in columnas else ('url' if 'url' in columnas else None)))
        
        if not col_cat:
            st.error(f"❌ No se encontró una columna de categoría en tu tabla '{tabla_actual}'. Columnas existentes: {columnas}")
            conn.close()
            return []
            
        # 3. Estructurar la consulta con fallbacks seguros
        query_cols = [col_id, col_nombre]
        query_cols.append(col_subcat if col_subcat else "''")
        query_cols.append(col_enlace if col_enlace else "''")
        
        # Limpiar emojis para buscar el texto puro
        cat_limpia = categoria_nombre.replace("📜","").replace("🏛️","").replace("💼","").replace("👥","").replace("⚖️","").replace("🛢️","").replace("⚡","").replace("💰","").strip()
        
        # Ejecutar con búsqueda flexible parcial (LIKE) para ignorar diferencias sutiles
        cursor.execute(f"SELECT {', '.join(query_cols)} FROM {tabla_actual} WHERE {col_cat} LIKE ?", (f"%{cat_limpia[:10]}%",))
        leyes = cursor.fetchall()
        conn.close()
        
        # Si sigue vacío, listamos las categorías reales de la base de datos para forzar coincidencia homóloga
        if not leyes:
            conn = sqlite3.connect(DB_NAME)
            c = conn.cursor()
            c.execute(f"SELECT DISTINCT {col_cat} FROM {tabla_actual}")
            cats_reales = [r[0] for r in c.fetchall() if r[0]]
            conn.close()
            
            for cr in cats_reales:
                if cat_limpia.lower()[:5] in cr.lower() or cr.lower()[:5] in cat_limpia.lower():
                    conn = sqlite3.connect(DB_NAME)
                    c = conn.cursor()
                    c.execute(f"SELECT {', '.join(query_cols)} FROM {tabla_actual} WHERE {col_cat} = ?", (cr,))
                    leyes = c.fetchall()
                    conn.close()
                    if leyes:
                        break
        ...
        st.error(f"💥 Error técnico en base de datos: {e}")
        return []

# FUNCIÓN AUTO-DETECTABLE ADAPTATIVA PARA EL BUSCADOR
def obtener_busqueda_especifica(texto_buscar):
    ...
    try:
        ...
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table'")
        tablas = [t[0] for t in cursor.fetchall() if t[0] != 'sqlite_sequence']
        tabla_actual = 'leyes' if 'leyes' in tablas else (tablas[0] if tablas else 'leyes')
        ...
        col_id = 'id' if 'id' in columnas else columnas[0]
        col_nombre = 'nombre' if 'nombre' in columnas else ('titulo' if 'titulo' in columnas else columnas[1])
        col_cat = 'categoria' if 'categoria' in columnas else ('area' if 'area' in columnas else columnas[2])
        col_enlace = 'enlace_pdf' if 'enlace_pdf' in columnas else ('enlace' if 'enlace' in columnas else ('link' if 'link' in columnas else ('url' if 'url' in columnas else col_id)))
        
        query = f"SELECT {col_id}, {col_nombre}, {col_cat}, {col_enlace} FROM {tabla_actual} WHERE {col_nombre} LIKE ? OR {col_cat} LIKE ?"
...
if busqueda:
    ...
    if resultados:
        ...
        with col_btn:
            if resultado_actual[3] and str(resultado_actual[3]).strip() != "": 
        ...
        st.warning("No se encontraron coincidencia para su búsqueda.")
else:
    st.session_state.indice_resultado = 0
...
if st.session_state.categoria_seleccionada:
    ...
    if leyes_encontradas:
        for fila in leyes_encontradas:
            id_ley = fila[0]
            nombre_ley = fila[1]
            subcat = fila[2] if len(fila) > 2 else "GENERAL"
            enlace = fila[3] if len(fila) > 3 else None
            ...
            st.markdown(f"""
                <div style="display:flex; justify-content:space-between; padding:10px 5px; font-weight:bold; color:#64748B; font-size:12px; border-bottom:2px solid #E2E8F0;">
                    <span>📌 {str(subcat).upper() if subcat else 'GENERAL'}</span>
                </div>
            """, unsafe_allow_html=True)
